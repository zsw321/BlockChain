...
// 从创世区块开始创建一个区块链实例，包含生成区块链对应的数据库
...
// 返回当前区块链实例下地址address所有的未花费的交易
// 1、遍历区块链数据库下的所有区块
// 2、从每个区块中遍历每笔交易
// 3、对每笔交易的输出进行判断，看address是否对应其锁定脚本，
//  同时找出每笔交易的输入是否有地址address的解锁脚本，若有，则收集对应的交易Id和输出索引号，即已花费的输出
	...
	for  {
		...
		Outputs:
			...
			for outIndex, out := range tx.Vout {
				...
				// 判断当前交易输出 是否有地址address，若是，则属于未花费的输出
...
// 获取当前区块链中地址address下所有的UTXO，即返回交易输出TXOutput数组，可用于计算当前地址的余额
// 1、获取地址address下区块链所有未花费的交易
...
// 从区块链实例中找到当前地址address下数量为amount的可花费的输出，
